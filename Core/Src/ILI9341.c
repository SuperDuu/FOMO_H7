...
#define ILI9341_RESX_HIGH()                                                    \
  SET_BIT(ILI9341_GPIO_PORT_RESX->BSRR, ILI9341_GPIO_PIN_RESX)
#define ILI9341_RESX_LOW()                                                     \
  SET_BIT(ILI9341_GPIO_PORT_RESX->BSRR,                                        \
          ((uint32_t)ILI9341_GPIO_PIN_RESX << 16U))
...
#else
#define ILI9341_CSX_HIGH()                                                     \
  SET_BIT(ILI9341_GPIO_PORT_CSX->BSRR, ILI9341_GPIO_PIN_CSX)
#define ILI9341_CSX_LOW()                                                      \
  SET_BIT(ILI9341_GPIO_PORT_CSX->BSRR, ((uint32_t)ILI9341_GPIO_PIN_CSX << 16U))
#endif

#define ILI9341_DCX_HIGH()                                                     \
  SET_BIT(ILI9341_GPIO_PORT_DCX->BSRR, ILI9341_GPIO_PIN_DCX)
#define ILI9341_DCX_LOW()                                                      \
  SET_BIT(ILI9341_GPIO_PORT_DCX->BSRR, ((uint32_t)ILI9341_GPIO_PIN_DCX << 16U))
...
// TODO: add timeouts
#define ILI9341_CHECK_SPI(spi_handler_ptr)                                     \
  do {                                                                         \
...
#define ILI9341_READ_DISPLAY_ID                                                \
  0x04U                         /* Read display identification information */
...
static void lcd_WriteCmd(uint8_t cmd, const uint8_t *params,
                         uint32_t param_len);
...
static void lcd_MakeArea(uint32_t x_start, uint32_t x_width, uint32_t y_start,
                         uint32_t y_height);
static HAL_StatusTypeDef lcd_SPI_Send(SPI_HandleTypeDef *hspi, uint8_t *data,
                                      uint8_t len, uint32_t timeout);
// static HAL_StatusTypeDef lcd_SPI_Read(SPI_HandleTypeDef *hspi, uint8_t *data,
// uint8_t len, uint32_t timeout);
static HAL_StatusTypeDef lcd_SPI_SendRead(SPI_HandleTypeDef *hspi,
                                          uint8_t *rxData, uint8_t *txData,
                                          uint8_t len, uint32_t timeout);

void Draw_BoundingBox(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                      uint16_t color);
void RGB565_to_INT8(const uint8_t *src, int8_t *dst, uint32_t width,
                    uint32_t height);
static uint16_t lcd_RGB888toRGB565(uint32_t rgb888);

#if (ILI9341_USE_DMA_CMSIS == 1)
static void spi_transmit_dma(SPI_HandleTypeDef *hspi, const uint8_t *pData,
                             uint16_t Size);
...
void RGB565_to_UINT8(const uint8_t *src, uint8_t *dst, uint32_t width,
                     uint32_t height) {
...
void Crop_and_Convert_RGB565_to_RGB888(const uint8_t *src, uint8_t *dst,
...
/* ===========================================================================
 * Crop_and_Convert_Fast: Merge crop + RGB565->RGB888 in ONE pass.
 * Reads 2 pixels per 32-bit word. Skips bit-fill for speed.
 * Eliminates the intermediate frame_192x192 buffer entirely.
 * ===========================================================================
 */
/* Crop 240x240 at screen center, resize to 128x128, RGB565 -> int8 RGB */
void Crop_and_Convert_Fast(const uint8_t *src, uint8_t *dst) {
  const uint32_t start_x = (SRC_WIDTH - CROP_WIDTH) / 2; // (320 - 240) / 2 = 40
  const uint32_t start_y =
      (SRC_HEIGHT - CROP_HEIGHT) / 2; // (240 - 240) / 2 = 0

  for (uint32_t y = 0; y < DST_HEIGHT; y++) {
    /* 240 / 128 = 15 / 8: fast scaling without integer division */
    const uint32_t src_y = start_y + ((y * 15U) >> 3);
    const uint32_t row_offset = src_y * SRC_WIDTH;
    int8_t *d = (int8_t *)(dst + y * DST_WIDTH * 3);
...
void LCD_PrintStringColor(int x, int y, const char *str,
...
extern volatile int ai_detection_count;
extern volatile float ai_peak_score;
extern volatile int8_t ai_peak_tgt, ai_peak_bg;
extern volatile int ai_peak_gx, ai_peak_gy;
extern volatile int ai_cells_over_bg, ai_cells_over_conf;
// extern float best_score ;
void Parse_SSD_Single_Output(float *data[]) {
  float *class_scores = data[0];
  float *bboxes = data[2];
  float best_score = 0.0f; /* Fixed: was uninitialized! */
  int best_x = 0, best_y = 0, best_w = 0, best_h = 0;

  /* Optimized: pointer arithmetic instead of index * 2 / index * 4 */
  float *cs = class_scores + 1; /* point to class 1 score */
  float *bb = bboxes;
  for (int i = 0; i < NUM_BOXES; i++, cs += 2, bb += 4) {
    float p_obj = *cs;
    if (p_obj < CONF_THRESHOLD)
      continue;
    ...
    int bw = (int)((x_max - x_min) * MODEL_INPUT_W);
    int bh = (int)((y_max - y_min) * MODEL_INPUT_H);
    ...
    /* LỌC NHIỄU: Bỏ qua các box quá lớn (model bị lỗi nhận diện toàn màn hình
  ...
  if (best_score > CONF_THRESHOLD) {
    ...
    x_ofs = best_x + best_w * 1.0f / 2 - 160;
    y_ofs = best_y + best_h * 1.0f / 2 - 120;
...
/* ===========================================================================
 * Parse_FOMO_Output: Parse 16x16x2 INT8 output tensor from X-CUBE-AI FOMO
 * model.
 * 1. 8-connectivity BFS clustering (gom mọi ô lân cận kể cả đường chéo).
 * 2. Weighted centroid (tâm khối lượng chính xác theo xác suất ô).
 * 3. Distance-based NMS (triệt tiêu cụm điểm trùng lặp trong bán kính 30px).
 * 4. Atomic IRQ protection (chống xung đột race-condition với ngắt DCMI).
 * ===========================================================================
 */
void Parse_FOMO_Output(const int8_t *output, uint32_t output_size) {
  if (output == NULL || output_size < (16 * 16 * 2)) {
    ...
    ai_peak_gy = 0;
    ai_cells_over_bg = 0;
    ai_cells_over_conf = 0;
    __enable_irq();
    return;
  }

  /* Compute telemetry stats across the entire 16x16 grid */
  int8_t max_t = -128, max_b = -128;
  int max_gx = 0, max_gy = 0;
  int over_bg = 0, over_conf = 0;
  for (int y = 0; y < 16; y++) {
    for (int x = 0; x < 16; x++) {
      int idx = (y * 16 + x) * 2;
      int8_t b_val = output[idx];
      int8_t t_val = output[idx + 1];
      if (t_val > b_val)
        over_bg++;
      float s = ((float)t_val + 128.0f) / 256.0f;
      if (s >= FOMO_CONF_THRESHOLD)
        over_conf++;
      if (t_val > max_t) {
        max_t = t_val;
        max_b = b_val;
 ...
 * Áp dụng Hysteresis Thresholding: Ô kích hoạt ban đầu phải >=
 * FOMO_CONF_THRESHOLD (0.80), nhưng các ô lân cận trong cùng vật thể chỉ cần >=
 * 0.40f là được gom chung vào một cụm duy nhất.
 */
#define FOMO_PROPAGATION_THRESHOLD 0.40f
       ...
       */
      float center_grid_x =
          (sum_w > 0.0f) ? (sum_x / sum_w) : ((float)(min_x + max_x) * 0.5f);
      float center_grid_y =
          (sum_w > 0.0f) ? (sum_y / sum_w) : ((float)(min_y + max_y) * 0.5f);
      int center_x = 40 + (int)(center_grid_x * 15.0f + 7.5f);
      int center_y = 0 + (int)(center_grid_y * 15.0f + 7.5f);
      if (center_x < 0)
        center_x = 0;
      if (center_x >= ILI9341_ACTIVE_WIDTH)
        center_x = ILI9341_ACTIVE_WIDTH - 1;
      if (center_y < 0)
        center_y = 0;
      if (center_y >= ILI9341_ACTIVE_HEIGHT)
        center_y = ILI9341_ACTIVE_HEIGHT - 1;

      /* Lọc bỏ cụm dị thường tràn màn hình */
      int span_w = (max_x - min_x + 1) * 15;
      int span_h = (max_y - min_y + 1) * 15;
      if (span_w > 180 || span_h > 180)
        continue;

      /* Lưu candidate và sắp xếp giảm dần theo điểm tin cậy (score) */
      if (cand_count < MAX_FOMO_CANDS) {
        int insert_pos = cand_count;
        for (int i = 0; i < cand_count; i++) {
          if (best_score > cands[i].score) {
            insert_pos = i;
            break;
          }
        }
        for (int i = cand_count; i > insert_pos; i--) {
          cands[i] = cands[i - 1];
        }
        cands[insert_pos].x = center_x;
        cands[insert_pos].y = center_y;
        cands[insert_pos].score = best_score;
        cand_count++;
      }
    }
  }
 ...
 * Bán kính triệt tiêu 60 pixel (~4 ô lưới, đường kính 120px bao phủ trọn vẹn 1
 * vật thể). Bất kỳ điểm nào nằm trong bán kính của một điểm mạnh hơn đều bị
 * triệt tiêu, ngăn chặn hoàn toàn việc sinh nhiều tâm xung quanh cùng 1 vật
 * thể.
 */
#define FOMO_SUPPRESS_DIST_SQ (60 * 60)
  ...
  /* Sao chép nguyên tử (Atomic transfer) có khóa ngắt để ISR không bao giờ đọc
   * trúng mảng dở dang */
  __disable_irq();
  ai_detection_count = final_count;
  ai_peak_tgt = max_t;
  ai_peak_bg = max_b;
  ai_peak_score = ((float)max_t + 128.0f) / 256.0f;
  ai_peak_gx = max_gx;
  ai_peak_gy = max_gy;
  ai_cells_over_bg = over_bg;
  ai_cells_over_conf = over_conf;
  for (int i = 0; i < 5; i++) {
    if (i < final_count) {
...
void Parse_SSD_Multi_Output(float *data[]) {
  ...
  for (int i = 0; i < detection_count; i++) {
    for (int j = i + 1; j < detection_count; j++) {
      ...
      int x_left = (detections[i].x > detections[j].x) ? detections[i].x
                                                       : detections[j].x;
      int y_top = (detections[i].y > detections[j].y) ? detections[i].y
                                                      : detections[j].y;
      int x_right = (detections[i].x + detections[i].width <
                     detections[j].x + detections[j].width)
                        ? detections[i].x + detections[i].width
                        : detections[j].x + detections[j].width;
      int y_bottom = (detections[i].y + detections[i].height <
                      detections[j].y + detections[j].height)
                         ? detections[i].y + detections[i].height
                         : detections[j].y + detections[j].height;
...
float calculate_iou(Detection det1, Detection det2) {
  ...
  int x_right = (det1.x + det1.width < det2.x + det2.width)
                    ? (det1.x + det1.width)
                    : (det2.x + det2.width);
  int y_bottom = (det1.y + det1.height < det2.y + det2.height)
                     ? (det1.y + det1.height)
                     : (det2.y + det2.height);
...
//     }
//
//     qsort(candidates, candidate_count, sizeof(Detection),
//     compare_detections);
//
...
void ILI9341_Init(SPI_HandleTypeDef *spi_handle,
  ...
  WRITE_REG(ILI9341.hspi->hdmatx->Instance->PAR,
            (uint32_t)&ILI9341.hspi->Instance->DR);
  __HAL_DMA_CLEAR_FLAG(ILI9341.hspi->hdmatx,
                       __HAL_DMA_GET_TC_FLAG_INDEX(ILI9341.hspi->hdmatx));
  __HAL_DMA_CLEAR_FLAG(ILI9341.hspi->hdmatx,
                       __HAL_DMA_GET_HT_FLAG_INDEX(ILI9341.hspi->hdmatx));
  __HAL_DMA_CLEAR_FLAG(ILI9341.hspi->hdmatx,
                       __HAL_DMA_GET_TE_FLAG_INDEX(ILI9341.hspi->hdmatx));
  __HAL_DMA_CLEAR_FLAG(ILI9341.hspi->hdmatx,
                       __HAL_DMA_GET_DME_FLAG_INDEX(ILI9341.hspi->hdmatx));
  __HAL_DMA_CLEAR_FLAG(ILI9341.hspi->hdmatx,
                       __HAL_DMA_GET_FE_FLAG_INDEX(ILI9341.hspi->hdmatx));
  /* Enable Common interrupts*/
  SET_BIT(ILI9341.hspi->hdmatx->Instance->CR,
...
uint8_t ILI9341_IsBusy(void) { return (ILI9341.buff_to_flush != NULL) ? 1 : 0; }

void ILI9341_DrawCrop(const uint8_t *buffer, uint32_t nbytes, uint16_t x1,
...
void *ILI9341_GetDrawBuffer1Addr(void) { return (void *)ILI9341.draw_buffer1; }

void *ILI9341_GetDrawBuffer2Addr(void) { return (void *)ILI9341.draw_buffer2; }

void ILI9341_SetBackgroundColor(uint32_t rgb888) {
  ILI9341_FillRect(rgb888, 0, ILI9341_ACTIVE_WIDTH, 0, ILI9341_ACTIVE_HEIGHT);
}

void ILI9341_FillRect(uint32_t rgb888, uint32_t x_start, uint32_t x_width,
  ...
  while (remaining_bytes) {
    ...
    bytes_sent_so_far +=
        lcd_CpyToDrawBuffer(ILI9341_PIXELS_TO_BYTES(npix), rgb888);
...
//     if (x + w > ILI9341_ACTIVE_WIDTH || y + h > ILI9341_ACTIVE_HEIGHT)
//     return;
//
//     // Tính toán địa chỉ đích trong framebuffer
//     uint32_t dest_addr = (uint32_t)OV7670.buffer_addr[0] + (y *
//     ILI9341_ACTIVE_WIDTH + x) * sizeof(uint16_t);
//
...
void ILI9341_FillRect_DMA2D(uint16_t color_rgb565, uint32_t x, uint32_t y,
                            uint32_t w, uint32_t h) {
  if (x >= ILI9341_ACTIVE_WIDTH || y >= ILI9341_ACTIVE_HEIGHT)
    return;
  if (x + w > ILI9341_ACTIVE_WIDTH)
    w = ILI9341_ACTIVE_WIDTH - x;
  if (y + h > ILI9341_ACTIVE_HEIGHT)
    h = ILI9341_ACTIVE_HEIGHT - y;
  if (w == 0 || h == 0)
    return;

  uint16_t *p_fb = (ili9341_active_fb != NULL)
                       ? (uint16_t *)ili9341_active_fb
                       : (uint16_t *)OV7670.buffer_addr[0];
...
void Draw_Rectangle_Outline(uint32_t x, uint32_t y, uint32_t width,
                            uint32_t height, uint16_t color_rgb565) {
  if (width == 0 || height == 0 || x + width > ILI9341_ACTIVE_WIDTH ||
      y + height > ILI9341_ACTIVE_HEIGHT)
    return;

  ILI9341_FillRect_DMA2D(color_rgb565, x, y, width, 1); // Cạnh trên
  ILI9341_FillRect_DMA2D(color_rgb565, x, y + height - 1, width,
                         1);                             // Cạnh dưới
  ILI9341_FillRect_DMA2D(color_rgb565, x, y, 1, height); // Cạnh trái
  ILI9341_FillRect_DMA2D(color_rgb565, x + width - 1, y, 1,
                         height); // Cạnh phải
}

/* Vẽ dấu crosshair '+' tại chính xác tâm vật thể (chuẩn FOMO) */
void Draw_Crosshair(uint32_t cx, uint32_t cy, uint32_t r,
                    uint16_t color_rgb565) {
  uint32_t x1 = (cx >= r) ? (cx - r) : 0;
  uint32_t x2 =
      (cx + r < ILI9341_ACTIVE_WIDTH) ? (cx + r) : (ILI9341_ACTIVE_WIDTH - 1);
  uint32_t y1 = (cy >= r) ? (cy - r) : 0;
  uint32_t y2 =
      (cy + r < ILI9341_ACTIVE_HEIGHT) ? (cy + r) : (ILI9341_ACTIVE_HEIGHT - 1);
...
/* Vẽ marker chuẩn FOMO: Crosshair tâm + ô vuông cố định kích thước size quanh
 * tâm */
void Draw_Target_Marker(uint32_t cx, uint32_t cy, uint32_t size,
                        uint16_t color_rgb565) {
  uint32_t half = size / 2;
  uint32_t x = (cx >= half) ? (cx - half) : 0;
  uint32_t y = (cy >= half) ? (cy - half) : 0;
  if (x + size >= ILI9341_ACTIVE_WIDTH)
    x = ILI9341_ACTIVE_WIDTH - size - 1;
  if (y + size >= ILI9341_ACTIVE_HEIGHT)
    y = ILI9341_ACTIVE_HEIGHT - size - 1;

  Draw_Rectangle_Outline(x, y, size, size, color_rgb565);
  Draw_Crosshair(cx, cy, half - 2, color_rgb565);
}

void ILI9341_Read_GRAM(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
                       uint8_t *buffer) {
  // Calculate the total number of pixels
  uint32_t num_pixels = (x2 - x1 + 1) * (y2 - y1 + 1);
  // placeholder for 2 18-bits RGB666 pixels, since ILI9341 is able to return
  // only in this format
  ...
  while (num_pixels > 0U) {
    lcd_SPI_SendRead(ILI9341.hspi, pixels_data, pixels_data,
                     sizeof(pixels_data), HAL_MAX_DELAY);
    ...
    for (uint8_t i = 0U; i < sizeof(pixels_data); i += 3) {
      ...
      *buffer++ =
          (pixels_data[i + 1] & 0xFC) | (pixels_data[i + 1] >> 6); // Green
      *buffer++ =
          (pixels_data[i + 2] & 0xFC) | (pixels_data[i + 2] >> 6); // Blue
    }
...
static void lcd_WriteDma(uint32_t src_addr, uint32_t nbytes) {
  ...
  ILI9341.spi_dma.src_address =
      src_addr + ILI9341_DMA_MAX_ITEMS *
                     (ILI9341.spi_dma.n_chunks - ILI9341.spi_dma.chunk_cnt) *
                     ILI9341_DATAWIDTH;
  ILI9341.spi_dma.chunk_cnt--;

  ILI9341_CSX_LOW(); // SPI CS
#if (ILI9341_USE_DMA_CMSIS == 1)
  spi_transmit_dma(ILI9341.hspi, (uint8_t *)ILI9341.spi_dma.src_address,
                   chunk_size);
#else
  HAL_SPI_Transmit_DMA(ILI9341.hspi, (uint8_t *)ILI9341.spi_dma.src_address,
                       chunk_size);
#endif
}
...
static void lcd_MakeArea(uint32_t x_start, uint32_t x_width, uint32_t y_start,
  ...
  ILI9341.area.x2 =
      (ILI9341.area.x2 > lcd_total_width) ? lcd_total_width : ILI9341.area.x2;
  ILI9341.area.y2 =
      (ILI9341.area.y2 > lcd_total_height) ? lcd_total_height : ILI9341.area.y2;
}
...
static uint8_t *lcd_GetBuff(void) {
  ...
  } else if ((uint32_t)ILI9341.buff_to_flush == buf1 &&
             ILI9341.buff_to_draw == NULL) {
    retPtr = ILI9341.draw_buffer2;
  } else if ((uint32_t)ILI9341.buff_to_flush == buf2 &&
             ILI9341.buff_to_draw == NULL) {
    retPtr = ILI9341.draw_buffer1;
  }
  __enable_irq();
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) {
  ...
  if (chunk_cnt == 0U) {
    ...
    src_address +=
        ILI9341_DMA_MAX_ITEMS * (n_chunks - chunk_cnt) * ILI9341_DATAWIDTH;
    chunk_cnt--;
  ...
  if (ILI9341.dma_cplt_cb != NULL) {
    ...
    ILI9341
        .dma_cplt_cb(); // <- is not defined and not used in current application
  }
